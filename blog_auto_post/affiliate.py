...
**2026-08-20: トリファ提携終了**。バリューコマースより「海外eSIM No.1アプリ トリファ
プログラム」が2026-08-25で終了する旨の通知を受領。以後の記事生成でトリファのアフィリエイト
リンクを新規に貼らないよう、VC_PID_MAPから除外した(`to_affiliate_url("トリファ", ...)`は
raw_urlへフォールバックする=素の公式サイトリンクになり、追跡なしのリンク切れないURLを返す)。
既に公開済みの記事に埋め込まれていたリンクは`fix_trifa_link_removal.py`で公式サイトURLへ
一括置換済み(2026-08-20実行、19記事)。plans.json・topics.jsonのトリファ関連データは
履歴として残しているが、今後のトピック生成では使わないこと(airalo・Saily 2社体制へ移行)。
"""
...
# 【2026-08-20】トリファは提携終了(2026-08-25)のためマップから除外済み(上記docstring参照)。
VC_PID_MAP: dict[str, str] = {
    "airalo": "892657920",
    "Saily": "892657921",
    # 【2026-09-21】高単価の併載用に即時提携(社長「勝手に即時提携してもいい」)。
    # 株式会社ビジョン「海外WiFiレンタルのグローバルWiFi」(プログラムID 2143810)。
    # 1件¥1,320(税込)でeSIM(¥100〜200)の約7〜10倍、承認率86.84%・平均EPC¥4.80。
    # リスティング(検索広告)でのブランド名出稿は禁止。ブログ本文での紹介は可。
    "グローバルWiFi": "892709163",
}
