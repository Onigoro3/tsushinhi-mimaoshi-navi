...
実装完了・ドライラン検証済み。`DEMON_DRY_RUN=true` での実行により、実際のプラン実データ
(mineo/IIJmio/UQ mobile/ワイモバイル/LINEMO/ahamo/BIGLOBEモバイル/NURO光/auひかり、
計9件)を使った記事生成が正常に動作することを確認済みです。**本番投稿(はてなブログへの
実際の公開)はまだ行っていません**(社長確認後に別途実施)。詳細は
`../developer/tasks.md`「## パイプライン実装・ドライラン検証 [2026-07-13]」参照。
...
- [ ] **ブログタイトルの設定**: はてなブログ管理画面(`magicaidemon.hatenablog.com`)の
      「設定」→「基本設定」→「ブログ名」を「通信費見直しナビ」に変更する
      (ドメイン変更は不要。Dragon「型落ち家電ラボ」の前例踏襲)
...
- [ ] **GitHubリポジトリの新規作成**: Dragon/Angelとは別リポジトリを作成し、本ディレクトリの
      中身をpushする(GitHub Actionsによる自動実行は未着手。必要であれば別途対応)。
      Secretsに上記環境変数一覧を登録する
